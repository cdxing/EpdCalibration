...
OUTDIR = Path("data/derived/candidate_v2")
OUT = OUTDIR / "candidate_162_213.csv"
...
EARLY_QA_FILES = {
    (162, 168): Path("data/derived/qa_162_168_review_v3/period_qa.csv"),
    (169, 175): Path("data/derived/qa_169_175_v2/period_qa.csv"),
    (176, 182): Path("data/derived/qa_176_182_v3/period_qa.csv"),
}

PERIOD_REFERENCES = Path("data/derived/period_references.csv")
...
early_qa = {
    period: index_qa(path)
    for period, path in EARLY_QA_FILES.items()
}

period_reference_rows = read_csv(PERIOD_REFERENCES)
period_references = {}
for r in period_reference_rows:
    period = (int(r["period_start"]), int(r["period_end"]))
    if period not in {(183, 186), (188, 189)}:
        continue
    key = (r["ew"], r["pp"], r["tt"])
    assert (period, key) not in period_references, (period, key)
    period_references[(period, key)] = r

assert sum(1 for period, key in period_references if period == (183, 186)) == 744
assert sum(1 for period, key in period_references if period == (188, 189)) == 744
...
# ----------------------------------------------------------------------
# 162–182: reviewed period-QA constants
# ----------------------------------------------------------------------
for period in [(162, 168), (169, 175), (176, 182)]:
    qa_path = EARLY_QA_FILES[period]

    for key, row in early_qa[period].items():
        if key in DEAD:
            assert row["n_used"] == "0", (period, key, row["n_used"])
            assert not row["constant"], (period, key, row["constant"])
            ...
            out.append(make_row(
                ...
                notes="Known dead tile; excluded from period fit.",
            ))
            continue
...
# ----------------------------------------------------------------------
# 183–186 and 188–189: Eloy period references
# Day187 has no usable calibration input and is represented in gap manifest.
# ----------------------------------------------------------------------
for period in [(183, 186), (188, 189)]:
    for ew in ("0", "1"):
        for pp in map(str, range(1, 13)):
            for tt in map(str, range(1, 32)):
                key = (ew, pp, tt)
                row = period_references[(period, key)]
                ...
                if key in DEAD:
                    out.append(make_row(
                        ...
                        status="known_dead_tile",
                        qa_flag="",
                        source_qa=str(PERIOD_REFERENCES),
                        calibration_source="none",
                        source_artifact=row["source_file"],
                        notes="Known dead tile; period reference value not used as calibration.",
                    ))
                    continue

                assert float(row["reference_mpv"]) > 0.0, (
                    period, key, row["reference_mpv"]
                )

                out.append(make_row(
                    period,
                    key,
                    calibration_mpv=row["reference_mpv"],
                    constant_error=row["reference_mpv_error"],
                    offset="0",
                    status="provisional",
                    qa_flag=row["auto_flag"],
                    source_qa=str(PERIOD_REFERENCES),
                    calibration_source="period_reference",
                    source_artifact=row["source_file"],
                    notes=(
                        f"Imported {row['source']} period reference "
                        f"for Days {period[0]}-{period[1]}."
                    ),
                ))
...
# Early periods:
# 162–168, 169–175, 176–182, 183–186, 188–189
# contribute 5 * 744 rows.
# Existing 190–208 candidate contributes 1500 rows.
# 211, 212, 213 contribute exactly 744 rows each.

expected_rows = 5 * 744 + 1500 + 3 * 744
assert len(out) == expected_rows, (
    f"expected {expected_rows} candidate rows, got {len(out)}"
)

for period in [(162,168),(169,175),(176,182),(183,186),(188,189)]:
    rr = [
        r for r in out
        if r["period_start"] == period[0]
        and r["period_end"] == period[1]
    ]
    assert len(rr) == 744, (period, len(rr))
...
# Explicit no-new-calibration / unresolved-input gaps.
gap_rows = [
    {
        "start_day": "187",
        "end_day": "187",
        "status": "no_usable_calibration_input",
        "reason": "No usable Day187 calibration input",
        "action": "no_new_calibration;previous_DB_state_persists",
    },
    {
        "start_day": "209",
        "end_day": "210",
