...
void th1to2() 
{
 ...
 // }
  ...
  TFile* out = new TFile(Form("data2d%dto%d.root",days[0],days[nday-1]),"RECREATE");
  //std::cout << "test 0.1 " << std::endl;
    ...
    //th3  = new TH3I(Form("RunAdcTile"),
        //Form("RunAdcTile;Tile ;ADC value;Run "),744,0.5,744.5,nNumber,0.5,0.5+nNumber,600,0,600);
    ...
    for (int ew=0; ew<2; ew++){
      ...
      th2[ew][pp-1][tt-1]  = new TH2F(Form("AdcEW%dPP%dTT%d",ew,pp,tt),
        ...
        Form("AdcEW%dPP%dTT%d;ADC value;days",ew,pp,tt),600,0,600,nday,0.5,0.5+nday);
        }
      }
    }
  ...
  { // iday
  	in	= new TFile(Form(
            "../data/Day%d.root",days[iday]),"READ");
            //"../runs/%d.picoDst.root",runs[iday]),"READ");
	std::cout << iday <<" runing run: " << days[iday] << std::endl;
	std::string sday = std::to_string(days[iday]);
	...
	//int itt = 1;
	  ...
	  { // pp
		...
		th2[ew][PP-1][TT-1]->GetYaxis() ->SetBinLabel(iday+1, pchar);
		...
		//	std::cout << "iadc +1: " << iadc + 1 << std::endl;
		  ...
		  th2[ew][PP-1][TT-1] ->Fill(iadc+1,iday+1,(Double_t)adc->GetBinContent(iadc+1));
